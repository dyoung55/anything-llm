...
export default function SlashCommandsTab({
  ...
  // In always-on agent mode (workspace.chatMode === "agent"), never show /exit.
  const items = useMemo(() => {
    const isAgentMode = workspace?.chatMode === "agent";
    const builtIn =
      !isAgentMode && isActiveAgentSession
        ? {
            command: "/exit",
  ...
  }, [workspace?.chatMode, isActiveAgentSession, presets, t]);
