...
## Custom Features Tracking

This repo maintains `CUSTOM_FEATURES.md` to document all local modifications so they survive upstream merges. **When you make a change that adds, modifies, or removes a custom feature, you must update `CUSTOM_FEATURES.md` in the same commit.** This includes:

- New database fields or schema changes
- New or modified API endpoints
- New UI components or modified behavior on existing screens
- New system prompt variables
- New boot-time migrations
- Any change to a file listed in the "Files to Check" sections of `CUSTOM_FEATURES.md`

If a change is purely upstream-tracking (e.g., a bug fix in an unmodified file), no update is needed.
